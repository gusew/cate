...
TEST(test_alg_bv_5tpl_classify)
{
  ...
  std::unique_ptr<Bitvector5tpl> alg(new Bitvector5tpl);
  try {
...
TEST(test_alg_bv_5tpl_classify_big)
{
  MemChronoSetup setup;
  AlgTestFixtures::setupMemChrono(setup);
...
TEST(test_alg_bv_5tpl_ruleadd_empty)
{
  MemChronoSetup setup;
  AlgTestFixtures::setupMemChrono(setup);
  ...
  assert_equal(indices[0], noRuleIsMatching(), SPOT); // if this line makes the test fail, 
  // check include/algorithms/DataBitvector.hpp: line 86
...
TEST(test_alg_bv_5tpl_ruleadd_filled)
{
  MemChronoSetup setup;
  AlgTestFixtures::setupMemChrono(setup);
...
TEST(test_alg_bv_5tpl_ruleremoved)
{
  MemChronoSetup setup;
  AlgTestFixtures::setupMemChrono(setup);
